...
Find your account id: it's printed at the bottom of Settings, under the
attribution line. Then set it as an environment variable in Vercel (Project →
...
Several admins: comma-separate them. Nobody listed means no admin, which is
the right default for a build that doesn't need one.
